...
pub fn main() void {
    ...
    const meal = food_loop: for (menu) |food| {
        ...
        i_loop: for (food.requires, 0..) |required, required_ingredient| {

            // This ingredient isn't required, so skip it.
            if (!required) continue;

            // This ingredient IS required! check if it's wanted
            for (wanted_ingredients, 1..) |want_it, idx| {
                if (required_ingredient == want_it) {
                    continue :i_loop;
                } else if (idx >= wanted_ingredients.len) continue :food_loop;
            }
        }
